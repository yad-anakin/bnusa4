...
const WhyChooseBnusa = () => {
  ...
  const reasons = [
    ...
    },
    {
      ...
      gradient: 'from-amber-50 to-yellow-50 shadow-amber-200/20'
    },
    {
      id: 4,
        ...
        {reduceMotion ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {reasons.map((reason) => (
              <div 
                key={reason.id} 
                className={`bg-gradient-to-br ${reason.gradient} p-8 rounded-2xl shadow-lg hover:shadow-xl transition-shadow`}
              >
          ...
          >
            {reasons.map((reason) => (
              <motion.div 
                key={reason.id} 
                className={`bg-gradient-to-br ${reason.gradient} p-8 rounded-2xl shadow-lg hover:shadow-xl transition-all`}
