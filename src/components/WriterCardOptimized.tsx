...
const WriterCardOptimized = ({ writer, rank }: WriterCardProps) => {
  ...
  return (
    <div className={`group relative bg-white/20 backdrop-blur-lg rounded-xl overflow-hidden 
      ${rank ? `border-2 ${rankStyles[rank].border} ${rankStyles[rank].shadow}` : 'border border-white/30'} 
      hover:border-[var(--primary)]/30 transition-all duration-300 
      ${rank ? 'shadow-xl' : 'shadow-lg'} hover:shadow-xl
      ${rank === 1 ? 'scale-105 z-10' : rank === 2 ? 'scale-102 z-5' : ''}`}>
      
      {/* Premium rank indicator (only for top 3) */}
      {rank && (
        <>
      ...
      <div className="p-6 relative z-10">
        ...
        <div className="flex items-center mb-5">
          <div className={`w-18 h-18 rounded-full overflow-hidden bg-gradient-to-br ${avatarGradient} relative flex items-center justify-center shadow-lg 
        ...
        <Link 
          ...
          className={`flex items-center justify-center w-full text-center py-3 px-5 rounded-lg transition-all duration-300 ${currentStyle.textColor} ${currentStyle.borderColor} border-2 ${currentStyle.hoverBg} hover:text-white font-medium text-sm shadow-sm hover:shadow-md backdrop-blur-sm`}
        >
      ...
      <div className={`h-1.5 w-full bg-gradient-to-r ${rank ? rankStyles[rank].badge : avatarGradient} opacity-70 absolute bottom-0 shadow-lg`}></div>
