...
export default function SettingsPage() {
    ...
    const fetchUserData = async () => {
      ...
      try {
        ...
        if (data.success && data.user) {
          // Log the user data we received, particularly the image paths
          console.log('Profile image path:', data.user.profileImage);
          console.log('Banner image path:', data.user.bannerImage);
          console.log('User image data:', data.user.userImage);
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>, type: 'profileImage' | 'bannerImage') => {
    ...
    try {
      ...
      console.log('Got auth token for image upload');
      
      // Use the utility function for uploads instead of manual implementation
      let uploadedImageUrl;
      if (type === 'profileImage') {
        console.log('Uploading profile image using uploadProfileImage utility');
        uploadedImageUrl = await uploadProfileImage(file, token);
      } else {
        console.log('Uploading banner image using uploadBannerImage utility');
        uploadedImageUrl = await uploadBannerImage(file, token);
      }
      
      console.log(`${type} uploaded successfully:`, uploadedImageUrl);
  ...
  const fetchUserProfile = async () => {
    try {
      ...
      if (userData.success && userData.user) {
        console.log('Refreshed user profile data:', userData.user);
        setFormData(prev => ({
          ...prev,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      for (let attempt = 1; attempt <= 3; attempt++) {
        try {
          console.log(`Updating MongoDB profile (attempt ${attempt})...`);
          ...
          if (data.success) {
            console.log(`Successfully updated profile in MongoDB (attempt ${attempt}):`, data);
            mongoUpdateSuccess = true;
            break;
          } else {
            console.error(`Failed to update profile in MongoDB (attempt ${attempt}):`, data);
            if (attempt < 3) await new Promise(r => setTimeout(r, 500 * attempt));
          }
        } catch (err) {
          console.error(`Error updating profile in MongoDB (attempt ${attempt}):`, err);
          if (attempt < 3) await new Promise(r => setTimeout(r, 500 * attempt));
        }
      }
      ...
      try {
        console.log('Attempting to update Firebase profile with:', { 
          name: formData.name, 
          ...
          <div className="flex flex-col md:flex-row gap-6">
            ...
            <div className="flex-1">
              <form onSubmit={handleSubmit} className="space-y-8">
                ...
                <div id="profile-images" className="bg-white rounded-lg shadow-sm p-6">
                  ...
                  <div className="space-y-6">
                      ...
                      <div className="flex flex-col md:flex-row items-center gap-6">
                        <div className="relative w-32 h-32 rounded-full overflow-hidden border-4 border-white shadow-lg">
                          {isLoading ? (
                            ...
                            <ImageWithFallback
                              src={formData.userImage?.profileImage || formData.profileImage || (currentUser?.photoURL || '')}
                              alt={formData.name}
                              fill
                    ...
                    <div className="mb-10">
                      ...
                      <div className="flex flex-col items-center gap-6">
                        <div className="relative w-full h-36 rounded-lg overflow-hidden border-2 border-gray-200 shadow-md">
                          {isLoading ? (
                            <div className="w-full h-full bg-gray-200 animate-pulse"></div>
                          ) : (
                            <ImageWithFallback
                              src={formData.userImage?.bannerImage || formData.bannerImage || '/images/default-banner.jpg'}
                              alt="Banner"
                              fill
                              style={{ objectFit: 'cover' }}
                ...
                <div id="social-media" className="bg-white rounded-lg shadow-sm p-6">
                ...
                <div id="account-actions" className="bg-white rounded-lg shadow-sm p-6">
