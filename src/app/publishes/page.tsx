...
export default function PublishesPage() {
  ...
  const fetchArticles = async (page: number) => {
    try {
      // Log whether this is a search or regular fetch
      if (!debouncedSearchQuery) {
        console.log(`Fetching default articles for page ${page}, category: ${activeCategory}`);
      } else {
        console.log(`Fetching articles with search for page ${page}, category: ${activeCategory}, query: "${debouncedSearchQuery}"`);
      }
      ...
      console.log(`Received ${data.articles?.length || 0} articles for page ${page}`);
      
      if (data.success) {
        // Set articles from the response
        ...
        if (debouncedSearchQuery.trim() && data.articles?.length === 0) {
          ...
          console.log('No results found for search query, showing no results message');
  ...
  useEffect(() => {
    ...
    if (searchQuery === '' && !loading) {
      console.log('Search query cleared - resetting to show all articles');
