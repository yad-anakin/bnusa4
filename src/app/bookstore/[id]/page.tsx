...
export default function BookPage({ params }: { params: { id: string } }) {
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      await api.get(`/api/books/${params.id}?download=true`, {});
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-[var(--primary)]/5 via-white to-[var(--primary)]/5 flex justify-center items-center">
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-[var(--primary)]/5 via-white to-[var(--primary)]/5">
        <div className="container mx-auto px-4 py-24">
          <div className="max-w-2xl mx-auto bg-white/40 rounded-xl p-6 text-center">
            <h1 className="text-2xl font-bold text-red-600 mb-4">کێشەیەک ڕوویدا</h1>
            <p className="text-[var(--grey-dark)] mb-6">{error || 'کتێبەکە نەدۆزرایەوە'}</p>
            <button
              onClick={() => router.back()}
              className="px-6 py-3 bg-[var(--primary)]/10 text-[var(--primary)] rounded-xl border border-[var(--primary)]/20 hover:bg-[var(--primary)]/20"
            >
              گەڕانەوە
            </button>
          </div>
          ...
          <div className="lg:col-span-2">
            <div className="space-y-8">
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-6">
                ...
                <div className="bg-white/40 rounded-xl p-4 border border-[var(--primary)]/10">
                  <h3 className="text-sm font-medium text-[var(--grey-dark)] mb-1">زمان</h3>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    {relatedBooks.map((relatedBook) => (
                      <Link href={`/bookstore/${relatedBook._id || relatedBook.id}`} key={relatedBook._id || relatedBook.id}>
