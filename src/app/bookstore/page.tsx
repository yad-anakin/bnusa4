...
export default function Bookstore() {
  ...
  const [selectedLanguage, setSelectedLanguage] = useState('هەموو');
  ...
  useEffect(() => {
    async function fetchBooks() {
      try {
        ...
        // Convert Kurdish language to English for API
        if (selectedLanguage !== 'هەموو') {
          const englishLanguage = languageMapping[selectedLanguage] || selectedLanguage;
          params.append('language', englishLanguage);
        }
        ...
        if (data.success) {
          ...
          if (data.filters) {
            ...
            if (data.filters.languages) setAvailableLanguages(data.filters.languages);
          }
        } else {
    ...
    fetchBooks();
  }, [pagination.page, debouncedSearchQuery, selectedGenre, selectedYear, selectedLanguage]);
      ...
      <div className="container mx-auto px-4 py-24">
        ...
        <div className="flex flex-wrap gap-4 justify-center mb-12">
          ...
          <select
            ...
            className="px-6 py-3 rounded-xl border border-white/20 bg-white/10 backdrop-blur-md focus:outline-none focus:border-[var(--primary)]/50"
          >
            <option value="هەموو">هەموو زمانەکان</option>
            {availableLanguages.map((language) => {
              // Convert English language to Kurdish for display
              const kurdishLanguage = englishToKurdishLanguage[language.toLowerCase()] || language;
              return (
                <option key={language} value={kurdishLanguage}>
                  {kurdishLanguage}
                </option>
              );
            })}
