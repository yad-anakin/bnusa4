...
<p><em>دەقی لار</em> و <strong>دەقی تۆخ</strong> دەتوانیت بەکار بهێنیت.</p>`);
  ...
  const handleImageUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      console.log(`Found ${cachedUrls.length} cached images, need to upload ${filesToUpload.length} new images`);
  ...
  const handleAddYoutubeLink = () => {
    ...
    console.log('YouTube links after adding:', JSON.stringify(updatedLinks));
  ...
  const handleAddResourceLink = () => {
    ...
    // Log the updated array for debugging
    console.log('Resource links after adding:', JSON.stringify(updatedLinks));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log('**DEBUG ARRAYS BEFORE SUBMISSION**');
      console.log('YouTube Links length:', cleanYoutubeLinks.length);
      console.log('YouTube Links data:', JSON.stringify(cleanYoutubeLinks));
      console.log('Resource Links length:', cleanResourceLinks.length);
      console.log('Resource Links data:', JSON.stringify(cleanResourceLinks));
      ...
      console.log('Article data to submit:', JSON.stringify({
        ...articleData,
        content: `${articleData.content.substring(0, 50)}...`, // Truncate content for logging
        youtubeLinks: articleData.youtubeLinks,
        resourceLinks: articleData.resourceLinks
      }));
      ...
      console.log('Article submission response:', data);
      
      // Verify the arrays in the response
      if (data.article) {
        console.log('Response article YouTube links:', JSON.stringify(data.article.youtubeLinks));
        console.log('Response article Resource links:', JSON.stringify(data.article.resourceLinks));
      }
  ...
  if (!currentUser) {
    return (
      <div className="container mx-auto py-16 relative overflow-hidden">
        ...
        {/* Subtle decorative icons - blue only */}
        <div className="absolute top-12 right-[10%] text-blue-500/10 transform rotate-12">
          <svg xmlns="http://www.w3.org/2000/svg" className="h-20 w-20" fill="currentColor" viewBox="0 0 24 24">
            <path d="M9.5 3a7.5 7.5 0 0 0 0 15h6a7.5 7.5 0 0 0 0-15h-6ZM9 16.5a6.5 6.5 0 1 1 0-13h7a6.5 6.5 0 0 1 0 13H9Z"/>
            <path d="M12 6.5c-.69 0-1.25.56-1.25 1.25S11.31 9 12 9s1.25-.56 1.25-1.25S12.69 6.5 12 6.5Z"/>
            <path d="M14 10.75h-4L8.75 14h6.5L14 10.75Z"/>
          </svg>
        </div>
        
        <div className="absolute bottom-14 left-[15%] text-blue-500/10 transform -rotate-12">
          <svg xmlns="http://www.w3.org/2000/svg" className="h-16 w-16" fill="currentColor" viewBox="0 0 24 24">
            <path d="M17.5 4.5c-1.95 0-4.05.4-5.5 1.5-1.45-1.1-3.55-1.5-5.5-1.5-1.45 0-2.99.22-4.28.79C1.49 5.62 1 6.33 1 7.14v11.28c0 .81.49 1.52 1.22 1.85 1.3.57 2.84.79 4.28.79 1.95 0 4.05-.4 5.5-1.5 1.45 1.1 3.55 1.5 5.5 1.5 1.45 0 2.99-.22 4.28-.79.73-.33 1.22-1.04 1.22-1.85V7.14c0-.81-.49-1.52-1.22-1.85-1.3-.57-2.83-.79-4.28-.79zM21 17.23c0 .63-.58 1.09-1.22 1.3-1.03.35-2.3.53-3.78.53-1.95 0-4.05-.4-5.5-1.5V8.5c1.45-1.1 3.55-1.5 5.5-1.5 1.45 0 2.75.15 3.78.48.65.21 1.22.67 1.22 1.3v8.45z" />
          </svg>
        </div>
          ...
          <div className="bg-white rounded-xl shadow-lg overflow-hidden border border-gray-100">
            <div className="p-8 md:p-10">
              <div className="flex items-start mb-8">
                ...
                <div className="pr-6">
                  <h2 className="text-2xl font-bold mb-4 text-gray-800">پێویستە هەژمارت هەبێت</h2>
                  <p className="text-gray-600 mb-6">
                    نووسینی وتار تەنها بۆ ئەو بەکارهێنەرانە بەردەستە کە چوونە ژوورەوەیان کردووە. چوونە ژوورەوە بکە یان هەژمارێکی نوێ دروست بکە بۆ ئەوەی دەستت بە ئیش بگات و وتارەکەت بنووسیت.
                  </p>
              </div>
            </div>
              ...
              <div className="border-t border-gray-100 pt-8 mb-8">
                <div className="space-y-4">
                  <div className="flex items-center text-gray-600">
                    ...
                    <span className="mr-3">بەشداریکردن لە پلاتفۆرمی بنووسە</span>
                  </div>
                  ...
                  <div className="flex items-center text-gray-600">
                    ...
                    <span className="mr-3">نووسینی وتار و بڵاوکردنەوەی بیروڕاکانت</span>
                  </div>
                  ...
                  <span><strong>بیرکردنەوە و نووسینەکانت بە جیهان بناسێنە</strong> لە پرۆفایلەکەتدا ببینیت. ئێمە هەوڵ دەدەین وتارەکەت زوو بە زوو پێداچوونەوەی بۆ بکەین.</span>
                </div>
              </div>
              
              <div className="flex flex-col sm:flex-row gap-4">
                <Link href="/signin" className="w-full sm:w-auto px-8 py-3.5 bg-blue-600 text-white rounded-lg hover:bg-blue-700 hover:shadow-md transition-all duration-300 text-center font-semibold flex justify-center items-center">
                  <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 ml-2" viewBox="0 0 20 20" fill="currentColor">
                    <path fillRule="evenodd" d="M3 3a1 1 0 011 1v12a1 1 0 11-2 0V4a1 1 0 011-1zm7.707 3.293a1 1 0 010 1.414L9.414 9H17a1 1 0 110 2H9.414l1.293 1.293a1 1 0 01-1.414 1.414l-3-3a1 1 0 010-1.414l3-3a1 1 0 011.414 0z" clipRule="evenodd" />
                  </svg>
                  چوونە ژوورەوە
              </Link>
                <Link href="/signup" className="w-full sm:w-auto px-8 py-3.5 border-2 border-blue-600 text-blue-600 rounded-lg hover:bg-blue-50 transition-all duration-300 text-center font-semibold flex justify-center items-center">
                  <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 ml-2" viewBox="0 0 20 20" fill="currentColor">
