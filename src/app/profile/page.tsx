...
export default function ProfilePage() {
  ...
  // Force clear profile cache and refresh data
  useEffect(() => {
    if (currentUser) {
      // Clear browser cache for the profile image by requesting it with a new URL
      const forceClearProfileCache = async () => {
        try {
          // Clear API cache
          api.clearCache();
          
          // Force a fresh fetch of user data
          const freshData = await api.noCache.get('/api/users/profile', {});
          
          if (freshData.success && freshData.user) {
            console.log('Force refreshed profile data with timestamp:', Date.now());
            
            // Create a new image element to preload the fresh image
            const preloadImg = new Image();
            preloadImg.src = `${freshData.user.profileImage}?t=${Date.now()}`;
            ...
            setUser(prevUser => {
              ...
              return {
                ...prevUser,
                profileImage: freshData.user.profileImage
              };
            });
          }
        } catch (error) {
      ...
      // Run the force refresh once when component mounts
      forceClearProfileCache();
    }
  }, [currentUser]);
      ...
      try {
        ...
        if (data.success && data.user) {
          console.log("User data fetched successfully:", {
            ...
            articlesCount: data.user.articles?.length || 0,
            hasUserImageData: !!data.user.userImage,
            timestamp: Date.now()
          });
  ...
  const updateFollowingMap = async (users: any[]) => {
    ...
    try {
      ...
      const userIds = filteredUsers.map(user => {
          ...
          console.log("Invalid user object or ID format:", user);
          return null;
      }).filter(id => id != null && id !== '');
      
      if (userIds.length === 0) {
        console.log("No valid MongoDB ObjectIds to check follow status for");
        return;
      }
  ...
  const handleModalFollowToggle = async (userId: string) => {
    ...
    // Add this log to see which modal we're in
    console.log(`Modal follow toggle called. In followers modal: ${showFollowersModal}, in following modal: ${showFollowingModal}`);
  ...
  const handleShowFollowersModal = async () => {
    if (user && user.followers.length > 0) {
      setFollowersLoading(true);
      try {
        console.log("Preparing to show followers modal");
        ...
        if (currentUser) {
          ...
          const endpoint = isViewingSelf ? 
            ...
            `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5003'}/api/users/${user.username}`;
            
          console.log(`Fetching latest user data from ${endpoint}`);
          ...
          if (userResponse.ok) {
            ...
            if (userData.success) {
              console.log(`User data fetched successfully, followers: ${userData.user.followers?.length || 0}`);
        ...
        // Next, ensure we have our latest following list to properly mark followed users
        if (isViewingSelf) {
          console.log("Fetching latest following list to cross-reference with followers");
          await fetchFollowing();
        }
        ...
        // Get a list of all users the current user is following
        // This will allow us to accurately mark which followers are being followed
        console.log("Updating following map for all followers");
        const followingIds = followingData.map(user => typeof user === 'string' ? user : user._id);
        
        // Ensure we update the followingMap for all followers
        // This guarantees the follow/unfollow buttons show the correct state
        if (followersData.length > 0) {
          console.log(`Processing ${followersData.length} followers and ${followingIds.length} following ids`);
          
          // First ensure we get accurate follow statuses from the API
          await updateFollowingMap(followersData);
          
          // Then explicitly mark any followers who are also being followed
          const newFollowingMap = {...followingMap};
          ...
          followersData.forEach(follower => {
            const followerId = typeof follower === 'string' ? follower : follower._id;
            if (followingIds.includes(followerId)) {
              console.log(`User ${followerId} is in following list, marking as followed`);
              newFollowingMap[followerId] = true;
            }
          });
          
          // Update the following map with these explicit changes
          console.log("Setting final following map for followers modal:", Object.entries(newFollowingMap).length, "entries");
          setFollowingMap(newFollowingMap);
          
          // Double-check that our following users are properly marked
          followingIds.forEach(id => {
            if (id && !newFollowingMap[id]) {
              console.log(`Warning: User ${id} should be followed but isn't marked in the map, fixing`);
              setFollowingMap(prev => ({
                ...prev,
                [id]: true
              }));
        ...
        console.log("Followers modal preparation complete");
      } catch (error) {
  ...
  const handleShowFollowingModal = async () => {
    if (user && user.following.length > 0) {
      setFollowingLoading(true);
      try {
        console.log("Preparing to show following modal");
        ...
        if (currentUser) {
          ...
          console.log(`Fetching latest user data from ${endpoint} for following list`);
          const userResponse = await fetch(endpoint, {
            headers: {
              'Authorization': `Bearer ${token}`
            }
          ...
          if (userResponse.ok) {
            const userData = await userResponse.json();
            if (userData.success) {
              console.log(`User data fetched successfully, following: ${userData.user.following?.length || 0}`);
              // Update the user state with the latest data
              setUser(prev => {
                if (!prev) return null;
                return {
        ...
        if (followingData.length > 0) {
          console.log(`Processing ${followingData.length} following users`);
          ...
          followingData.forEach(user => {
            const userId = typeof user === 'string' ? user : user._id;
            newFollowingMap[userId] = true;
            console.log(`Marking user ${userId} as followed in following list`);
          });
          
          // Update the following map with these changes
          console.log(`Setting final following map with ${Object.entries(newFollowingMap).length} entries`);
          setFollowingMap(prev => ({...prev, ...newFollowingMap}));
        }
        
        console.log("Following modal preparation complete");
      } catch (error) {
  ...
  useEffect(() => {
    ...
    if (refreshParam) {
      console.log(`Found refresh parameter: ${refreshParam}, forcing page reload`);
  ...
  const getProfileImageUrl = (user: User) => {
    ...
    if (user.userImage?.profileImage) {
      console.log('Using profileImage from UserImage collection');
      // Add cache buster that's less aggressive
      return `${user.userImage.profileImage}?v=${new Date().toISOString().split('T')[0]}`;
    }
    
    // Check for Firebase Auth photoURL if available
    if (currentUser?.photoURL && !user.profileImage) {
      console.log('Using profileImage from Firebase Auth');
      return currentUser.photoURL;
    }
    
    // Fall back to user's direct profileImage field
    console.log('Falling back to profileImage from User model');
  ...
  const getBannerImageUrl = (user: User) => {
    ...
    if (user.userImage?.bannerImage) {
      console.log('Using bannerImage from UserImage collection');
      // Add cache buster that's less aggressive
      return `${user.userImage.bannerImage}?v=${new Date().toISOString().split('T')[0]}`;
    }
    
    // Fall back to user's direct bannerImage field
    console.log('Falling back to bannerImage from User model');
    return user.bannerImage || '/images/default-banner.jpg';
  };
    ...
    const burstImageCache = () => {
      if (user && user.userImage) {
        ...
        if (typeof window !== 'undefined') {
          ...
          console.log('Preloaded fresh images to burst cache');
        }
      }
    };
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="container mx-auto px-4 relative">
        <div className="bg-white rounded-lg shadow-sm -mt-20 p-6">
