  ...
  if (typeof window !== 'undefined') {
    try {
      ...
      if (storedMetadata) {
        ...
        console.log(`Restored ${Object.keys(metadata).length} image cache entries from localStorage`);
      }
      ...
      if (storedRegistry) {
        try {
          ...
          console.log(`Loaded image registry with ${Object.keys(imageRegistry).length} entries`);
        } catch (e) {
          console.warn('Error parsing image registry:', e);
          imageRegistry = {};
        }
      }
    } catch (error) {
    ...
    try {
      ...
      try {
        ...
        if (storedMetadata) {
          ...
          console.log(`Cleaned ${cleaned} old entries from localStorage image cache`);
        }
      } catch (e) {
        console.warn('Error cleaning localStorage caches:', e);
      }
      
      console.log(`Cache cleanup complete. Removed ${registryCount - Object.keys(imageRegistry).length} registry entries.`);
    } catch (e) {
...
const apiRequest = async (
  ...
  if (isImageRequest && method === 'GET') {
    ...
    if (imageUrl && isImageAlreadyLoaded(imageUrl)) {
      ...
      // console.log(`Using cached image (already loaded): ${imageUrl.substring(0, 50)}...`);
    ...
    if (!response.ok) {
      ...
      if (response.status === 429) {
        ...
        if (similarKeys.length > 0) {
          ...
          console.log("Using similar cached data due to rate limiting");
...
const api = {
  ...
  post: (endpoint: string, data: any, options: RequestInit = {}) => {
        ...
        for (const key in data) {
          ...
          if ((key === 'youtubeLinks' || key === 'resourceLinks') && value !== undefined) {
            if (Array.isArray(value)) {
              ...
              console.log(`API - Prepared ${key}:`, JSON.stringify(result[key]));
            } else {
              // If somehow not an array, create an empty array
              result[key] = [];
              console.log(`API - ${key} was not an array, initialized as empty array`);
            }
          }
    ...
    // Log specific array fields for debugging
    if (processedData) {
      if (processedData.youtubeLinks) {
        console.log('API sending youtubeLinks:', JSON.stringify(processedData.youtubeLinks));
      }
      if (processedData.resourceLinks) {
        console.log('API sending resourceLinks:', JSON.stringify(processedData.resourceLinks));
      }
    }
    ...
    console.log('API sending data:', JSON.stringify({
      ...enhancedData,
      content: enhancedData.content ? `${enhancedData.content.substring(0, 50)}...` : undefined
    }));
  ...
  clearCache: () => {
    ...
    console.log('API cache cleared, except for persistent image cache');
  },
  ...
  clearUserCache: () => {
    ...
    if (typeof window !== 'undefined') {
      try {
        ...
        console.log('Added image cache buster to clear browser cache');
      } catch (error) {
      ...
      try {
        ...
        if (storedMetadata) {
          ...
          if (metadata[imageUrl]) {
            ...
            localStorage.setItem(IMAGE_CACHE_KEY, JSON.stringify(metadata));
            console.log(`Refreshed cache for image: ${imageUrl}`);
          }
        }
      } catch (error) {
  ...
  uploadImage: async (file: File, folder: string = 'articles') => {
    try {
      ...
      if (cachedUrl) {
        console.log('Using cached image URL from previous upload:', cachedUrl);
